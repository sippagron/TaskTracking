...
public class AddGeneralController {
    ...
    public void handleUseSaveButton(ActionEvent actionEvent){
        try {
            ...
            ArrayList<GeneralWork> test = new FileManager().readFileGenaralList();
            int go = -1;
            int index = -1;
            boolean check = true;
            ...
            GeneralWork general = new GeneralWork(go,jobname.getText(),Integer.parseInt(priority.getText()),Integer.parseInt(timestart1.getText()),Integer.parseInt(timestart2.getText())
                    ,Integer.parseInt(timeend1.getText()),Integer.parseInt(timeend2.getText()),a,category.getText());
            FileManager fileManager = new FileManager();
            fileManager.saveFileGeneralList(general);
            fileManager.readFileGenaralList();
            FXRouter.goTo("generalwork");
        } catch (IOException e) {
            System.err.println("ไปที่หน้า generalwork ไม่ได้");
            System.err.println("ให้ตรวจสอบการกำหนด route");
