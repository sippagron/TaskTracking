...
public class AddRegularController {
    ...
    public void handleUseSaveButton(ActionEvent actionEvent) {
        try {
            ...
            ArrayList<RegularWork> test = new FileManager().readFileRegularList();
            int go = -1;
            int index = -1;
            boolean check = true;
            ...
            RegularWork regular = new RegularWork(go,jobname.getText(),Integer.parseInt(priority.getText()),Integer.parseInt(timestart1.getText()),Integer.parseInt(timestart2.getText())
                    ,Integer.parseInt(timeend1.getText()),Integer.parseInt(timeend2.getText()),a,category.getText());
            FileManager fileManager = new FileManager();
            fileManager.saveFileRegularList(regular);
            fileManager.readFileRegularList();
            FXRouter.goTo("regularwork");
        } catch (IOException e) {
            System.err.println("ไปที่หน้า regularwork ไม่ได้");
            System.err.println("ให้ตรวจสอบการกำหนด route");
